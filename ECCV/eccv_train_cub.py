...
class CustomDataset(torch.utils.data.Dataset):
    ...
    def get_image_files(self, data_dir, data_type):
        image_files = []
        # Read train_test_split.txt
        with open(os.path.join(data_dir, "train_test_split.txt"), "r") as file:
            train_test_split = file.readlines()

        # Read images.txt
        with open(os.path.join(data_dir, "images.txt"), "r") as file:
            image_lines = file.readlines()

        images_dir = os.path.join(data_dir, "images")
        for line in train_test_split:
            image_id, is_train = map(int, line.strip().split())
            if (data_type == "train" and is_train) or (
                data_type == "test" and not is_train
            ):
                file_path = os.path.join(
                    images_dir, image_lines[image_id - 1].strip().split()[1]
                )
...
def save_images(img, original, grayscale, colorized):
    ...
    folder_path = os.path.join(os.getcwd(), f"eccv_trained_cub")
...
# Training function
def train_eccv16(model, train_loader, device, num_epochs=100, lr=1e-4):
    ...
    writer = SummaryWriter(log_dir="runs/eccv16_cub")
    ...
    for epoch in range(num_epochs):
        ...
        if epoch % 10 == 0:
            ...
            torch.save(model.state_dict(), f"eccv16_cub_{epoch}.pth")

    # Close the SummaryWriter object
    writer.close()

    # Save training metrics summary to a text file
    with open("eccv_cub_train.txt", "w") as f:
...
if __name__ == "__main__":
    ...
    train_dataset = CustomDataset("CUB_200_2011", "train")
    ...
    # Train the model
    train_eccv16(model, train_loader, device, num_epochs=100)

    # Save the trained model
    torch.save(model.state_dict(), "eccv16_cub_trained.pth")
