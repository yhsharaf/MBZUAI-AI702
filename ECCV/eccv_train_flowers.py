...
def save_images(img, original, grayscale, colorized):
    ...
    folder_path = os.path.join(os.getcwd(), f"eccv_trained_flowers")
...
def train_eccv16(model, train_loader, device, num_epochs=100, lr=1e-4):
    ...
    writer = SummaryWriter(log_dir="runs/eccv16_flowers")
    ...
    for epoch in range(num_epochs):
        ...
        if epoch % 10 == 0:
            ...
            torch.save(model.state_dict(), f"eccv16_flowers_{epoch}.pth")

    # Close the SummaryWriter object
    writer.close()

    # Save training metrics summary to a text file
    with open("eccv_flowers_train.txt", "w") as f:
...
if __name__ == "__main__":
    ...
    torch.save(model.state_dict(), "eccv16_flowers_trained.pth")
