...
def save_images(img, original, grayscale, colorized):
    ...
    folder_path = os.path.join(os.getcwd(), f"siggraph_trained_cub")
...
def train_siggraph17(model, train_loader, device, num_epochs=100, lr=1e-4):
    ...
    writer = SummaryWriter(log_dir="runs/siggraph17_cub")
    ...
    for epoch in range(num_epochs):
        ...
        if epoch % 10 == 0:
            ...
            torch.save(model.state_dict(), f"siggraph17_cub_{epoch}.pth")

    # Close the SummaryWriter object
    writer.close()

    # Save training metrics summary to a text file
    with open("siggraph_cub_train.txt", "w") as f:
...
if __name__ == "__main__":
    ...
    torch.save(model.state_dict(), "siggraph17_cub_trained.pth")
