...
def save_images(img, original, grayscale, colorized):
    ...
    folder_path = os.path.join(os.getcwd(), f"siggraph_test_cub")
...
# Evaluation function
def evaluate_siggraph17(model, test_loader, device):
    ...
    with open("siggraph_cub_eval.txt", "w") as f:
...
if __name__ == "__main__":
    ...
    test_dataset = CustomDataset("CUB_200_2011", "test")
    ...
    model.load_state_dict(torch.load("siggraph17_cub_trained.pth"))

    # Evaluate the model
    evaluate_siggraph17(model, test_loader, device)
